...
**Model selection (issue #12):** the batch-translation strategy is unchanged. The concrete Gemini model is now user-configurable (`UserSettings.geminiModel`, default `gemini-2.5-flash`) rather than hardcoded `gemini-2.0-flash`.
