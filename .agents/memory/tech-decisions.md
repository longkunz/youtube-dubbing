...
- **ADR-0001 (amended) + ADR-0010**: Default topology is a Self-hosted Backend (CPU Docker, EN→VI + Piper then Edge TTS on the server). Chrome does not open Bing WebSocket. BYOK Gemini/OpenAI/Groq remain optional.
...
- **ADR-0007 (Pluggable Translation)**: Gemini, OpenAI-compatible `/v1/chat/completions`, or Self-hosted Backend `POST /v1/translate`. YouTube Caption Translation stays outside `TranslationClient` (ADR-0009).
